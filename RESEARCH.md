...
6. **SSE 스트리밍의 post-yield 코드는 위험하다.** Starlette의 `StreamingResponse`는 cancel scope 안에서 실행되므로, `yield` 이후 코드가 cancel되어 실행되지 않을 수 있습니다. 비동기 후처리는 반드시 `BackgroundTask`로 분리해야 합니다. 이 발견이 Sub-B 파이프라인 전체를 살렸습니다.
...
1. **NPC 이름 정규화 미구현**: LLM이 같은 NPC를 다른 이름으로 출력하면 중복 엔트리 생성 (예: "세라핀 (어머니)" vs "세라핀 (원거리에서 목소리만)")
