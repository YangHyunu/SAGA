...
응답이 유저에게 전달된 **뒤에** `BackgroundTask`로 실행됩니다. `asyncio.Event` 락으로 이전 턴 처리 완료를 보장합니다.

#### 파이프라인 (7단계)

1. **State Block 파싱** — 정규식으로 12필드 추출. 닫는 ` ``` ` 없는 블록도 처리
2. **Flash 폴백** — 정규식 실패 시 경량 LLM(Gemini Flash)으로 JSON 추출
3. **NPC 이름 정규화** — 기존 캐릭터와 매칭. 한국어 존칭 제거 + 접미사 매칭
4. **SQLite 상태 갱신** — characters, relationships, locations, world_state, events
5. **턴 로그 기록** — turn_log 테이블에 전체 I/O 저장
6. **ChromaDB 에피소드 기록** — 요약 + 중요도 + 분류 + 엔티티 태깅
7. **live_state.md 갱신** — player_context 쿼리 → 원자적 파일 교체

#### State Block

LLM에게 응답 끝에 출력하도록 요청하는 12필드 구조화 블록:
...
items_gained: [불꽃 검]
items_lost: []
items_transferred: [{item: 치유 물약, to: 에르겐}]
npc_met: [고블린왕 크룩]
npc_separated: []
relationship_changes: [{from: 아리아, to: 크룩, type: hostile, delta: -2}]
mood: tense
event_trigger: 고블린 습격
notes: 크룩이 다음 턴에 도주 시도할 가능성
```
````

유저에게는 State Block이 제거된 깨끗한 응답만 전달됩니다.
...
1. **서사 모순 탐지** — SQLite 규칙 기반 (HP≤0 생존 등) + 에이전트 판단 (위치 불일치, 죽은 NPC 재등장 등)
...
SAGA는 고비용 모델(Sonnet)은 내레이션에만, 경량 LLM(Flash)은 추출 폴백에만 사용합니다. 정규식 파싱이 성공하면 Flash 호출도 발생하지 않습니다 (E2E 검증에서 23/23 regex 성공). 그리고 .md 캐시 + 3-BP Prompt Caching으로 입력 토큰 비용을 추가 76% 절감합니다.
...
1. **추출 정확도**: 모든 것이 State Block 파싱 품질에 달려 있습니다. LLM이 State Block을 잘못 출력하면 상태가 오염됩니다. 경량 LLM 폴백(Gemini Flash)으로 완화하지만 완벽하지 않습니다.
...
3. **State Block 의존은 양날의 검**입니다. 구조화된 추출이 가능하지만, LLM이 출력을 실수하면 모든 것이 무너집니다. 정규식 파싱을 1차로, Gemini Flash를 2차로 두는 이중 폴백이 실전에서 23/23 성공률을 보여줬습니다.
...
- **Sub-B 추출**: 23/23 regex 성공 (Flash 폴백 0회)
- **Curator**: 턴 10, 20에서 실행 — NPC 중복 탐지(세라핀 4개 엔트리), 위치 모순 탐지
