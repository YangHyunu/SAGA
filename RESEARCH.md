...
| 모드 | 평균 히트율 | 50턴 비용 | 절감률 |
|------|:----------:|:---------:|:------:|
| **수동 3-BP** | **95.5%** | **$0.17** | **76.7%** |
| 자동 top-level | 10.0% | $0.83 | -15.0% |
| 캐시 없음 | 0% | $0.72 | 기준선 |

> **결론: 장기 RP 대화에서는 수동 3-BP가 필수.** 자동 캐싱은 20턴 이상에서 오히려 비용이 증가한다.
> 벤치마크 스크립트: `tests/bench_prompt_caching.py` | 상세 데이터: `tests/bench_report_3bp.json`
