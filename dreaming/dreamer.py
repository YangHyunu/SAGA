...
- 한 턴에 서술 분량이 많으면 사실이 여러 개 섞여 있다. 대사·핵심 사건에 가려진
  배경 진술(예: "가끔 ~한 적이 있다"류의 과거 행적·습관 묘사)도 빠짐없이 각각
  별도 fact로 추출하라.
- 사물이 언급되면 그 사물을 특정하는 색상·재질·용기 같은 부수 묘사도 fact에
  포함하라. 예: "자색 유리병에서 ~이 나왔다"라면 "~을 소유하고 있다"처럼
  핵심 행위만 남기지 말고 용기의 색상·재질도 함께(또는 별도 fact로) 기록하라.
...
- key_excerpts에는 큰따옴표(")나 줄바꿈이 포함된 문장을 고르지 마라 — JSON을
  깨뜨린다. 적합한 문장이 없으면 빈 배열로 두라.
