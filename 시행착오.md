...
| 지표 | 수동 3-BP | 자동 top-level |
|------|----------|---------------|
| 평균 히트율 (턴 5+) | **95.5%** | 10.0% |
| 총 비용 | **$0.17** | $0.83 |
| 절감률 | **76.7%** | -15.0% (더 비쌈) |
...
3. **SAGA의 3-BP 설계가 정확히 맞는 전략** — 명시적 breakpoint로 시스템/중간/마지막 지점을 고정하므로 대화 길이에 무관

### 벤치마크 스크립트
- `tests/bench_prompt_caching.py` — 3-BP / 자동 / no-cache 모드 지원
- `tests/bench_report_3bp.json` — 수동 3-BP 50턴 raw 데이터
- `tests/bench_report_auto.json` — 자동 캐싱 50턴 raw 데이터
