...
### 코퍼스 #3 (트림)·#4 (리롤/편집) — ✅ 수집 완료 (2026-08-06)

**#3 트림** (`dreaming_data/corpus3/`, 8턴, maxContext 50000):
- 30000은 불가 — 카드 고정분(~48K t)이 최소 요구를 넘는다. 50000이 트림 발동 스팟
- 트림은 **앞에서** 깎는다: greeting(4,651자)부터 소멸, corpus2 말미 48개 → 38개
- 턴 4~7이 46개 고정/감소 = **정상상태 트림** (매 턴 자란 만큼 잘림). 5회 이벤트
- 함의: 1안이 지키는 건 system까지. 채팅 구획은 트림마다 재편 — **Task 7(BP2·압축) 영역의
  실물 픽스처.** system도 132K→136K로 동시 변동(로어) — 두 파손이 겹치는 리얼 케이스

**#4 리롤/편집** (`dreaming_data/corpus4/`, 6요청):
- req-001→002: Δ-2 pop — **리롤 = 마지막 assistant 뽑고 재전송**
- req-002≡003: 48/48 **완전 동일 바이트** — 연속 리롤은 같은 요청 반복 (캐시 완전 히트)
- req-004→005: 공통 44/48 — **편집 = 중간 변이**, diverge 지점이 와이어에 그대로
- req-005→006: 공통 1/48 — 트림+새 턴이 겹치면 msg[0] 빼고 메시지 단위 공통성 붕괴
- 보너스: 이중전송 사고로 빈 유저 턴(`*says nothing*`) 포함 — 실유저 다발 패턴

**⚠️ 발견 — PairLedger 정합성 이상 (미수정, 구현자 몫):**
트림+리롤+편집 조합 후 `dreaming_data/default/raw/` 원장이 시간순이 아니다 —
turn 24(시간상 마지막) 뒤에 turn 25(corpus3 옛 페어 재기록), turn 26(편집 **전** 원문).
편집된 텍스트는 원장에 없음. **트림 정상상태에서 resolver의 턴 정렬이 흔들리는 실물 증거.**
`identity.py` PairLedger 도메인 — corpus3/4를 픽스처로 재현 가능.

## 0-2. 재생 A/B — 트림 정상상태에서의 1안 + Anthropic 마킹 라이브 검증 (2026-08-06) ✅

corpus3(트림 8턴) 동일 바이트를 haiku-4.5(OpenRouter)로 두 번 재생.
A = 마킹만 주입(1안 없음, 직접 호출) / B = dreaming(:8787) 통과 = **lore_shift + mark_cache 실코드 라이브**.

| | A: 마킹만 | B: 1안+마킹 |
|---|---|---|
| 완전 미스 턴 | 4/8 | **0/8** |
| 총비용 (8턴) | $0.383 | **$0.180 (2.1×↓)** |
| 무캐시($0.428) 대비 | -10% | **-58%** |

- **마킹만 켜면 캐시가 오히려 손해인 턴이 생긴다** — 완전 미스 턴 $0.0659 vs 무캐시 $0.0528
  (쓰기 프리미엄 1.25×). "로어북 안 고치고 마킹만 = 돈 더 씀" 실측
- dreaming 실코드의 Anthropic 경로 라이브 검증 완료 — `task_a92d91d8` 마킹 절반 해소
- **신규 발견 — 제2 mutable 존 = 상태창.** B에서 4턴이 38,423t 지점에서 파손:
  keyed 로어를 빼도 시스템 안 상태창(시간·날씨·소지금 — "786년 10월 04일 08:31 AM…")이
  바뀌며 그 지점부터 깨진다. 로어북 다음 서열의 범인. 처방은 SystemStabilizer 사상
  (동적 델타를 프리픽스 밖으로) — **1안 확장 후보 #1**
- 관찰: B의 usage에서 `cache_creation_input_tokens`가 0으로 보고됐으나 과금 역산 시
  write 프리미엄 실재 — OpenRouter 보고 스키마 문제로 추정, 결론 무관
- 방법론 확정: **새 코퍼스 = 브라우저 1회 채굴, 검증 = 재생 무한.** 위지소연/HypaV3
  라이브 코퍼스 계획은 재생으로 대체 가능해 취소 (변수 3개 동시 변경이라 대조군도 아니었음)
