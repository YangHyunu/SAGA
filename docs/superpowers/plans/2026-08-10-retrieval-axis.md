...
**2026-08-10 스모크 결과 — 통과.** fix-drm-r0 원문 12턴(T1025~1036)을 스크래치
세션에 복사, DeepSeek V4 Flash 실호출 1사이클: 에피소드 3개 / key_excerpts 9개 —
9개 전부 원문 substring 검증 통과, fact 28개, 사이클 폐기 0. 따옴표·줄바꿈 회피
규칙(e60d35a)이 실출력에서 지켜져 JSON 파싱 무손상. 특기: T59 실패 케이스의
생강·꿀 문장("이 산에서 나는 생강과 꿀을 섞어 말린 것이네. 몸을 덥히는 데
좋아.")이 발췌로 원문 박제됨 — 어휘 검색이 못 잡는 패러프레이즈를 발췌 축이
보완하는 설계 의도의 첫 실증. 주의: 호출 ~6분 소요(Flash 추론 폭주 계열) — 유휴
실행이라 비차단이지만, 프로덕션 Dreamer에도 eval 하네스처럼 추론 예산 캡 도입
필요.
