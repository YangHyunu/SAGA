...
### 2.3 Haystack #2 — 출처 편향 교차 검증 (2026-08-11 추가)

§8 미결 질문 3(원문 출처 편향)의 실험적 해소책: **vanilla 출신 런을 두 번째 건초더미로 추가**한다.

- 후보 실물 확인 완료: `dreaming_data/eval/v2-fix-van-r0-run0.json` — 100턴 완주,
  프로브 9개(턴 19~99), ledger 343개 (2026-08-11 검증, annyeong 워크트리).
- 근거: vanilla 런은 주입·압축이 전혀 없던 런이라 어떤 메모리 시스템의 지문도
  없는 순수 서사다. dreaming 출신 haystack A와 vanilla 출신 haystack B 양쪽에서
  변형 순위가 일치하면 출처 편향 논란이 실험적으로 닫히고, 갈리면 그 자체가
  발견(haystack 민감성)이다.
- 규칙: 두 haystack의 점수를 **평균 내지 않는다** — 다른 대본이므로 별도 보고.
  비용은 프로브 9 × 5변형 × 2건초더미 = 라이브 콜 90회 + 유상태 인제스천
  2세트(haystack당 1회 상각).
...
3. **원문 진행 구간의 출처 편향**: §2.1에서 "원출처 변형이 무엇이든 무관하다"고 가정했는데, 이 haystack은 원래 dreaming 나레이터가 dreaming 변형의 컨텍스트를 보고 쓴 글이다 — 나레이터가 (인지하지 못해도) 자기 응답 스타일에 어떤 식으로든 압축된 컨텍스트의 흔적을 남겼을 가능성을 완전히 배제할 수 있는지는 실험적으로 검증되지 않았다. → **해소책 확정: §2.3 Haystack #2(vanilla 출신) 교차 검증** (2026-08-11).
