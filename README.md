...
3. [동기] Sub-A: .md 캐시 읽기 -> ChromaDB 에피소드 검색 + SQLite 로어 조회 -> 토큰 예산 내 조립
4. Anthropic cache_control: ephemeral (1h TTL) 적용
5. LLM 1회 호출
6. SSE 스트리밍으로 클라이언트에 응답 반환
7. 턴 카운터 증가
8. [비동기] Sub-B: Flash 서사 요약 -> ChromaDB 에피소드 기록 -> NPC 레지스트리 갱신 -> live_state.md 갱신
9. [비동기, N턴마다] Curator: 모순 탐지 -> 서사 압축 -> 로어 자동생성
```
...
슬라이딩 윈도우가 발동하면 RisuAI가 히스토리를 트리밍하면서 prefix가 바뀌어 캐시가 깨진다. max context를 넉넉히 잡으면 트리밍이 발동하지 않아 즉시 해결된다. 리버스 프록시 제약으로 트리밍 시점을 직접 제어할 수 없는 한계가 있지만, 실용적으로는 컨텍스트 크기 설정으로 해결된다.
