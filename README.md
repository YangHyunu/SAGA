...
| 에피소드 기억 | RRF 랭킹된 관련 에피소드 (최대 10개, 개별 200자 cap) |
| 활성 로어 | SQLite + 벡터 검색 병합 (최대 5개, 개별 400자 cap) |
...
> **핵심**: Sub-A = **읽기 전용** (DB→컨텍스트), Sub-B = **쓰기 전용** (Flash 서사 요약→에피소드 기록→NPC 레지스트리), LLM 호출은 **Proxy가 직접** 수행한다. Sub-B와 Curator는 Proxy가 독립적으로 `asyncio.create_task()`하며, Sub-B → Curator 종속 관계는 없다.
...
8. [비동기] Sub-B: Post-Turn
   → Flash 서사 요약 (4필드) → ChromaDB 에피소드 기록 → NPC 레지스트리 갱신 → turn_log 기록
9. [비동기, N턴마다] Curator
   → 모순 탐지 → 서사 압축 → 로어 자동생성
```
...
Sub-A의 결과는 `{"md_prefix": str, "dynamic_suffix": str}`로 반환된다. Anthropic 캐싱 경로에서는 `md_prefix`가 시스템 메시지 안에, `dynamic_suffix`가 마지막 user 메시지에 prepend된다. 비-Claude 경로에서는 `[--- SAGA Dynamic Context ---]` 헤더로 시스템 메시지에 합쳐진다.
...
[활성 로어]                     ← SQLite lore + ChromaDB 벡터 검색
### 어둠의 숲
에르시아 변방의 위험한 숲...
```
...
4. NPC 레지스트리 갱신
   ...
   ▼
5. SQLite 턴 로그 기록
   │  turn_log (narrative, user_input, response)
```
...
이 기능들은 `saga/lorebook/` 모듈로 분리하여 구현할 예정이다 (Phase 5 로드맵).
...
models:
  ...
  extraction: "gemini-2.0-flash"            # Flash 서사 요약 (경량 LLM)
...
saga/
  ...
  session.py               # 세션 관리자 (get_or_create, reset, 월드 데이터 로드)
  ...
  agents/
    ...
    post_turn.py           # Sub-B: Flash 서사 요약 + ChromaDB 에피소드 기록 + importance 스코어링
