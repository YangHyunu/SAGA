...
| **LongMemEval** (ICLR 2025) | 499 QA, 인스턴스당 ~53세션 | 21.2% | **63.5%** (+42.3%p) | 같은 10세션 예산, 검색만 다름 |

두 벤치마크 모두 동일 모델(gemini-2.5-flash) A/B 비교. SAGA가 더 정확한 이유는 모델 성능이 아니라 **필요한 정보가 컨텍스트에 있느냐**의 차이다.
...
10. [벤치마크: LOCOMO (ACL 2024)](#10-벤치마크-locomo-acl-2024)
11. [벤치마크: LongMemEval (ICLR 2025)](#11-벤치마크-longmemeval-iclr-2025)
12. [빠른 시작](#12-빠른-시작)
13. [설정 레퍼런스](#13-설정-레퍼런스)
14. [API 레퍼런스](#14-api-레퍼런스)
15. [월드 데이터 작성법](#15-월드-데이터-작성법)
16. [프로젝트 구조](#16-프로젝트-구조)
17. [로드맵](#17-로드맵)
18. [선행 연구 및 참고 자료](#18-선행-연구-및-참고-자료)
...
- **LLM-as-a-Judge** [[5]](#ref-5) -- LLM을 평가자로 사용하는 방법론. SAGA는 LOCOMO, LongMemEval [[6]](#ref-6) 벤치마크에서 LLM-as-Judge로 메모리 시스템 효과를 정량 평가한다.
...
- **에이전트 컨텍스트 관리의 .md 수렴** -- Claude Code, Cursor, oh-my-claudecode(OMC), Codex, Letta Code MemFS[[7]](#ref-7) 등 아키텍처가 다른 에이전트 시스템들이 공통적으로 마크다운을 컨텍스트 관리 형식으로 채택하고 있다. "에이전트가 무엇이 중요한지 스스로 판단하여 .md 메모를 갱신한다"는 패러다임에서 영감을 받아, SAGA의 .md 캐시 설계와 Letta Curator의 Memory Block 자기편집 구조로 이어졌다.

### 접근 방식

OpenAI-compatible 프록시로 클라이언트와 LLM 사이에 위치하여:

1. **매 턴 요청 시** -- 3종 스토리지에서 현재 상태와 관련 컨텍스트를 검색, 프롬프트에 주입 (동기, 밀리초 단위)
2. **매 턴 응답 후** -- LLM 응답에서 상태 변화를 추출, 3종 스토리지를 갱신 (비동기, 유저 대기 없음)
...
- **동기 경로**: DB 검색 + 프롬프트 조립만 수행 (LLM 호출 없음, 밀리초 단위)
...
1. **READ** (동기): Sub-A가 3종 스토리지에서 현재 상태, 관련 로어북을 검색하여 프롬프트에 주입
...
| **Letta Code** | 에이전트 프레임워크 | `memory/*.md` + YAML frontmatter (MemFS) [[7]](#ref-7) |
...
특히 Letta Code의 MemFS[[7]](#ref-7)는 주목할 만하다. Memory Block(단일 문자열)에서 진화하여, git 기반 마크다운 파일 시스템으로 에이전트 메모리를 관리한다 -- SAGA의 .md 캐시(`stable_prefix.md`, `live_state.md` + YAML frontmatter)와 매우 유사한 구조다. 차이는 **편집 주체**: MemFS는 에이전트가 LLM 호출로 편집하고, SAGA는 코드 로직(Sub-B)이 밀리초 단위로 편집한다.
...
```
1. 세션 ID 추출 (4단계 우선순위: @@SAGA: plugin sentinel -> X-SAGA-Session-ID 헤더 -> user 필드 -> 시스템 메시지 첫 paragraph (최대 300자) SHA256 해시 앞 16자)
2. 세션 존재 확인 -> 없으면 새 세션 생성 (SQLite + .md 캐시 초기화)
3. [동기] Sub-A: Context Builder (밀리초 단위)
...
> **역할**: 매 턴 동기 실행, 밀리초 단위 목표, LLM 호출 없음
...
3. ChromaDB 에피소드 기록
   ...
   │  importance >= 40: 중요 에피소드로 분류 (Important 검색 대상)
   ▼
4. NPC 레지스트리 갱신
...
6. live_state.md 갱신
   ...
   │  (주의: 플레이어 캐릭터 DB 등록 경로가 미구현이므로,
   │   query_player_context()가 기본값을 반환. live_state.md는 기본 상태로 갱신됨)
```
...
| 장기 서사 압축 | 50턴 이상 + stable_prefix.md가 비어있을 때 요약 압축 (두 조건 모두 충족 시에만 실행) |
...
| **characters** | session_id, name, is_player, hp, max_hp, location, mood, traits, custom | PC + NPC 상태 (NPC 이름 등록만 가능, HP/위치/기분 업데이트는 미구현) |
| **relationships** | session_id, from_name, to_name, rel_type, strength | NPC 관계 (스키마 정의됨, 쓰기 경로 미구현) |
| **locations** | session_id, name, description, first_visit_turn | 장소 (스키마 정의됨, 쓰기 경로 미구현) |
| **events** | session_id, name, event_type, description, turn, importance, entities | 세계 이벤트, 퀘스트 (스키마 정의됨, 쓰기 경로 미구현) |
...
| 작업 | 기본 모델 | 이유 |
|------|----------|------|
| 내레이션 | Claude Haiku 4.5 | 비용 효율적이면서 충분한 서사 품질 |
...
| 로어 자동생성 | Gemini 2.5 Flash Lite (extraction 모델 공유) | 저비용 구조화 출력, 큐레이션당 최대 3건 |
...
- 메모리 회상 효과: LOCOMO Judge +54%, LongMemEval 21%→63.5% (+42.3%p)
...
## 10. 벤치마크: LOCOMO (ACL 2024)

### 개요

[LOCOMO](https://github.com/snap-research/locomo) (ACL 2024)는 장기 대화 메모리 평가를 위한 산업 표준 벤치마크다. 10개 대화(총 5,882턴, 1,986 QA pairs)에서 5가지 카테고리의 질문으로 메모리 시스템의 정보 회상 능력을 평가한다.

**평가 방식**: 동일 모델 A/B 비교
- **Baseline**: 최근 60턴을 그대로 컨텍스트에 넣고 질문에 답변
- **SAGA**: 최근 10턴 + ChromaDB 벡터 검색으로 관련 메모리 에피소드를 주입하여 답변

### 결과 (LLM-as-Judge, 1-5점)

> 2개 대화, 304 QA pairs, QA/Judge 모델: gemini-2.5-flash-lite
...
**핵심 발견:**
- **multi-hop 4.71/5**: 여러 세션에 걸친 정보 종합이 거의 완벽. 이는 "lost in the middle" 문제를 메모리 시스템이 구조적으로 해결함을 보여준다.
- **10턴 + 메모리 > 60턴 raw context**: 적은 컨텍스트로 더 정확한 답변 = 토큰 비용 절감 + 품질 향상 동시 달성
- **약한 모델일수록 효과 극대**: 저가 모델에서 메모리 증강 효과가 더 극적으로 나타남 (flash-lite: +1.10, flash: +0.29)

### 실행 방법

```bash
# 1. LOCOMO 데이터셋 자동 다운로드 + Sub-B Ingestion + QA 평가 + Judge
...
결과는 `benchmarks/results/`에 JSON + Markdown 리포트로 저장된다.

---

## 11. 벤치마크: LongMemEval (ICLR 2025)

### 개요

[LongMemEval](https://huggingface.co/datasets/xiaowu0162/longmemeval-cleaned) (ICLR 2025)은 장기 대화 메모리 시스템의 5가지 능력을 평가하는 벤치마크다. 500개 QA 인스턴스, 각 인스턴스당 평균 53개 세션(~115K 토큰)으로 구성된다.

**테스트하는 5가지 메모리 능력:**

| 능력 | 설명 | 예시 |
|------|------|------|
| **single-session (user/assistant)** | 단일 세션에서 언급된 정보 회상 | "내 강아지 품종이 뭐야?" |
| **single-session-preference** | 사용자 선호도 기억 | "마이애미 호텔 추천해줘" (이전에 언급한 취향 기반) |
| **multi-session** | 여러 세션에 걸친 정보 종합 | "헤드폰이랑 아이패드 합쳐서 얼마 썼어?" |
| **temporal-reasoning** | 시간 순서 기반 추론 | "토마토 심은 게 먼저야, 오이 심은 게 먼저야?" |
| **knowledge-update** | 변경된 정보 반영 | "지금 직업이 뭐야?" (이직 후) |

**평가 방식**: 동일 모델, 동일 토큰 예산(10 sessions) A/B 비교
- **Baseline**: 마지막 10개 세션을 그대로 사용 (truncation)
- **SAGA**: 질문과 관련된 상위 10개 세션을 ChromaDB 벡터 검색으로 선별 (retrieval)
- **Judge**: LLM이 정답 포함 여부를 Yes/No로 판정
...
231건 개선 vs 20건 퇴보 (11.5:1 비율).

### 이 결과가 말하는 것

Baseline이 21%밖에 못 맞추는 건 모델이 약해서가 아니다. 53개 세션 중 마지막 10개에 정답이 포함될 확률이 낮기 때문이다. SAGA는 같은 10개 세션을 쓰되, **어떤 10개를 고르느냐**만 바꿔서 63.5%를 달성했다.

이건 모델 성능과 독립적인 개선이다. gemini-3-flash 같은 더 강한 모델을 써도, 컨텍스트에 정답이 없으면 답을 만들어낼 수 없다. 검색 전략의 가치는 모델 세대가 바뀌어도 유지된다.

**카테고리별 관찰:**
- **single-session-assistant (+69.6%p)**: assistant가 말한 정보는 대화 초반에 몰려 있어 truncation에 가장 취약. 검색으로 거의 완전히 해결됨.
- **temporal-reasoning (+41.7%p)**: 시간 순서 추론은 관련 세션이 모두 컨텍스트에 있어야 가능. 검색이 이를 보장.
- **single-session-preference (+16.7%p)**: 선호도 질문은 암묵적이라 벡터 검색으로 잡기 어려움. 개선 여지가 남아 있는 영역.

### 실행 방법

```bash
# 전체 500 인스턴스 평가 (체크포인트 자동 저장, 중단 후 재개 가능)
...
OpenAI API 키 필요 (text-embedding-3-small 임베딩). 결과는 `benchmarks/longmemeval/data/results/`에 저장.
...
saga/
  ...
  utils/
    __init__.py            # 패키지 초기화
    parsers.py             # JSON 파서 (잘린 JSON 복구, Flash 응답 파싱) + state block strip
...
tests/
  ...
  eval_llm_judge.py        # LLM-as-a-Judge 평가 스크립트 (deprecated, LOCOMO 벤치마크로 대체)
...
| Phase | 상태 | 내용 |
|-------|------|------|
| **Phase 1** | 완료 | 코어 프록시 + 3-Agent 파이프라인 + 3종 스토리지 |
...
<a id="ref-6"></a>
<a id="ref-6"></a>
**[6]** Wang, D., Zhu, Y., et al. (2025). *LongMemEval: Benchmarking Chat Assistants on Long-Term Interactive Memory.* ICLR 2025. -- 장기 대화에서 5가지 메모리 능력(단일 세션 추출, 다중 세션 추론, 시간 순서 추론, 지식 업데이트, 기권)을 평가하는 벤치마크. 인스턴스당 ~53세션(~115K 토큰)으로 구성. https://huggingface.co/datasets/xiaowu0162/longmemeval-cleaned

<a id="ref-7"></a>
**[7]** Letta. (2025). *MemFS: Memory as a File System.* Letta Documentation. -- 에이전트 메모리를 git 기반 마크다운 파일 시스템으로 관리하는 접근. Memory Block에서 진화하여 YAML frontmatter + 마크다운 파일 구조를 채택. SAGA의 .md 캐시 2파일 (stable_prefix + live_state) 설계의 직접적 참고점. https://docs.letta.com/letta-code/memory
